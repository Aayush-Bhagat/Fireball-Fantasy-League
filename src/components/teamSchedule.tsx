...
export default function TeamSchedule({ teamSchedule, isAdmin }: Props) {
    const router = useRouter();
    const [games, setGames] = React.useState<GameResponseDto["games"]>([]);

    React.useEffect(() => {
        teamSchedule.then((data) => setGames(data.games));
    }, [teamSchedule]);
            ...
            <div className="space-y-4">
                {games.map((game) => (
                    <div
                        ...
                        className="cursor-pointer flex flex-col sm:flex-row justify-between items-center border rounded-xl p-4 shadow hover:shadow-lg transition bg-white"
                    >
                        {/* Date & Time */}
