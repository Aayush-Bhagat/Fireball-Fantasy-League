...
function calculateCareerStats(
    seasons: CareerStatsDto[],
    sluggingSeasons: CareerStatsDto[],
) {
    /* -------------------------------------------------------
       BATTING
    ------------------------------------------------------- */
    ...
    const walksTaken = sumNullable(seasons, "walksTaken") ?? 0;
    ...
    const singles = sumNullable(sluggingSeasons, "singles");

    const doubles = sumNullable(sluggingSeasons, "doubles");

    const triples = sumNullable(sluggingSeasons, "triples");
    const sluggingAtBats = sumNullable(sluggingSeasons, "atBats");

    const sluggingHomeRuns = sumNullable(sluggingSeasons, "homeRuns");
    ...
    const obp = calculateOBP(hits, walksTaken, hitByPitch, atBats, sacFlies);
...
export default function FullPlayerStats({ player }: Props) {
    ...
    const sluggingSeasons = useMemo(() => {
        return playedSeasons.filter((season) => {
            const seasonNumber = season.seasonId;

            return seasonNumber !== null && seasonNumber >= 5;
        });
    }, [playedSeasons]);
    ...
    const careerStats = useMemo(
        () => calculateCareerStats(playedSeasons, sluggingSeasons),
        [playedSeasons, sluggingSeasons],
    );
